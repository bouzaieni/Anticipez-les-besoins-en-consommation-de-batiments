...
# # Notebook de modélisation de l'émission de GES
...
data = pd.read_csv('data_gaz_nettoye.csv')
...
# # 2. Modélisation Emission de GES
...
X_train, X_test, y_train, y_test = partition_train_test(data,'TotalGHGEmissions',0.2)
...
Je vais chercher les hyperparametres des 4 meilleurs modèles qui ont attiré mon attention (et, xgb, gb et lgb)
# In[28]:
...
# Grille de recherche
models_hyperparameters = {  et: {'max_depth': [10,50,100],  'n_estimators': [10, 50, 100],'min_samples_split' : [2, 5, 10],'min_samples_leaf' : [1, 3, 4]  },
                            gb: {'learning_rate': [0.01,0.1,0.5],'subsample' : [ 0.1,0.9],'n_estimators' : [100,500,1000],'max_depth': [1,10],'min_samples_split' : [2, 10],'min_samples_leaf' : [1, 3]},
                            xgb: {'max_depth': [5,10],'learning_rate':[ 0.1,0.5],'n_estimators': [100,500,1000] },
...
def Initialiser_hyperparameters(best_hyperparameters):
    et_opt =  ExtraTreesRegressor(**best_hyperparameters[et])
    gb_opt = GradientBoostingRegressor(**best_hyperparameters[gb])
    xgb_opt = XGBRegressor(**best_hyperparameters[xgb])
    lgb_opt = LGBMRegressor(**best_hyperparameters[lgb])
    models = [ et_opt, gb_opt ,xgb_opt, lgb_opt]
...
df_performances_par_defaut = df_performances_avec_normalisation.iloc[[9,11,12,13],:]
...
#  - Extra Tree
#  
# Une légère dégradation des résultats avec la variable ENERGYSTARScore
